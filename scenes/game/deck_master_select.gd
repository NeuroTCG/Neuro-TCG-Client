...
@onready var status_text = $Window/UI/TopRow/DeckMasterSelectText
...
func _on_confirm_button_clicked() -> void:
	var response: DeckMasterSelectedPacket = await (
		Global
		. network_manager
		. send_packet_and_await_response(
			DeckMasterRequestPacket.new(Packet.next_response_id(), card_ids[selected_id])
		)
	)
	assert(response.valid, "deck master select failed.")
	confirm_button.disabled = true
	confirm_button.text = "CONFIRMED"
	if not opponent_ready:
		status_text.text = "Waiting for opponent"


func _on_game_start(_packet: GameStartPacket) -> void:
