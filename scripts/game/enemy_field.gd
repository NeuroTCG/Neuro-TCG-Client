...
func _apply_ability(ability: Ability, target_position: CardPosition, target_card_state: CardState) -> void:
	...
	match ability.effect:
		...
		Ability.AbilityEffect.ATTACK when (
			ability.range == Ability.AbilityRange.ENEMY_CARD
		):
			var target_slot_no := Field.array_to_index(
			...
			target_card.take_damage(target_card_state.health - target_card.health)
			assert(target_card.state.shield == target_card.shield)
		Ability.AbilityEffect.ATTACK when (
			ability.range == Ability.AbilityRange.ENEMY_ROW
		):
			var target_slot_no := Field.array_to_index(
				target_position.to_array(), Field.Side.Player
			)
