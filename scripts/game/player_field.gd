...
func _verify_ability_or_magic(action: MatchManager.Actions, source: Card, target_slot: Array[int]) -> void:
	match action:
		MatchManager.Actions.ABILITY:
			VerifyClientAction.ability.emit(
				get_slot_array(source), target_slot
			)

		MatchManager.Actions.MAGIC:
			VerifyClientAction.magic.emit(
				source.state.id, null if target_slot == null else target_slot
			)
...
func _on_slot_chosen(slot_no: int, card: Card) -> void:
	...
	if MatchManager.current_action == MatchManager.Actions.ABILITY || MatchManager.current_action == MatchManager.Actions.MAGIC:
		assert(selected_slot.stored_card, "No card selected.")
		...
		if selected_slot.stored_card.info.ability.effect == Ability.AbilityEffect.ADD_HP:
			...
			_verify_ability_or_magic(MatchManager.current_action, card, get_slot_array(selected_slot.stored_card))
...
func _on_enemy_slot_chosen(enemy_slot_no: int, enemy_card: Card) -> void:
	# TODO: ugly hack to choose between card from field slot and hand slot
	# assert(selected_slot.stored_card, "No card selected.")
	var player_card := Global.player_hand.selected_card if selected_slot == null else selected_slot.stored_card
	...
	if MatchManager.current_action == MatchManager.Actions.ATTACK:
	...
	elif MatchManager.current_action == MatchManager.Actions.ABILITY || MatchManager.current_action == MatchManager.Actions.MAGIC:
		...
		_verify_ability_or_magic(MatchManager.current_action, player_card, index_to_array(enemy_slot_no))
